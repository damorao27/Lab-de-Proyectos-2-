...
{
//	Cambiamos de estado a la frecuencia del Timer:
		if (estado == 1)
		{estado = 2;}
/*	Estados:
 * 1. Espera
...
{
	//	Guardamos la Data recibida cuando el buffer este lleno:
	word Received;
	byte errr;
	errr = AS1_RecvBlock((byte*)&DTH11,2, &Received);
	busin=AS1_ClearRxBuf();
}
...
{
//	Guardamos la medida del ultrasonido
  err=Cap1_GetCaptureValue(&Medida);
}
